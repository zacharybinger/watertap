...
def initialize_mixer(m, guess=True):
    if guess:
        recovery_guess = 0.3
        m.fs.M1.inlet_2_state[0.0].flow_mass_phase_comp["Liq", "H2O"].fix(
            (1 - recovery_guess) * 0.965
        )
        m.fs.M1.inlet_2_state[0.0].flow_mass_phase_comp["Liq", "TDS"].fix(
            0.035 / (1 - recovery_guess)
        )
    m.fs.M1.initialize()
...
def optimize(m, water_recovery=0.1, Q_ro=0.965):
    ...
    m.fs.P1.control_volume.properties_out[
        0
    ].pressure.unfix()  # pump outlet pressure (Pa)                                # pump efficiency (-)
    m.fs.P2.control_volume.properties_out[
        0
    ].pressure.unfix()  # pump outlet pressure (Pa)
...
def print_results(m):
    ...
    print(
        f'MIXER CONC: {value(pyunits.convert(m.fs.M1.mixed_state[0].conc_mass_phase_comp["Liq", "TDS"], to_units=pyunits.g/pyunits.L)):<5.2f} {pyunits.get_units(pyunits.convert(m.fs.M1.mixed_state[0].conc_mass_phase_comp["Liq", "TDS"], to_units=pyunits.g/pyunits.L))}'
    )
    print("\n")
    print(
    ...
    print("\n")
    print(
        f"LCOW: {value(m.fs.costing.LCOW):<5.2f} {pyunits.get_units(m.fs.costing.LCOW)}"
    )
    print("\n\n")
