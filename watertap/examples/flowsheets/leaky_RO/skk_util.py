...
# def create_report(m):
#     var_labels = ["Water Permeability Coeff","Salt Permeability Coeff", "Feed Pressure", "Feed Mass Flow Rate", "Feed Solute Mass Flow Rate", "Feed Vol Flow Rate", "Feed Osm Pressure", "Perm Pressure", "Perm Mass Flow Rate", "Perm Solute Mass Flow Rate", "Perm Vol Flow Rate", "Perm Osm Pressure", "Perm Density", "Water Flux", "Salt Flux", "Reflection Coeff.", "Alpha"]
#     vars = [m.fs.A, m.fs.B, m.fs.feed[0].pressure(), m.fs.feed[0].flow_mass_phase_comp[liq, h2o], m.fs.feed[0].flow_mass_phase_comp[liq, tds], m.fs.feed_vol_flow, m.fs.feed[0].pressure_osm_phase[liq], m.fs.perm[0].pressure(), m.fs.perm[0].flow_mass_phase_comp[liq, h2o], m.fs.perm[0].flow_mass_phase_comp[liq, tds], m.fs.perm_vol_flow, m.fs.perm[0].pressure_osm_phase[liq], m.fs.perm[0].dens_mass_solvent(), m.fs.flux_mass_phase_comp, m.fs.salt_flux_mass_phase_comp, m.fs.reflect_coeff, m.fs.alpha]
        ...
        if blk.name == 'fs.ROunit_SKK_1D':
            row = [
                ...
                100* (1 - value(blk.mixed_permeate[0.0].flow_mass_phase_comp['Liq', 'NaCl'] / 
                                blk.feed_side.properties[0,0].flow_mass_phase_comp['Liq', 'NaCl']
                        )
                        ),
